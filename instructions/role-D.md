...
This role focuses on the factorial operation feature and fixing import issues.

## Starting Point

Your starter branch: `starter/role-D`
...
1. Checkout your starter branch (`starter/role-D`).
...
Task 1: Fix the failing tests (`tests/test_calculator.py`)

Look at the test output and the import statement at the top of `tests/test_calculator.py`. Figure out what is missing and add it.

Task 2: Update `.gitignore`

Add entries to `.gitignore` so that Python cache files and directories are excluded from version control.

### How to commit

Make these as two separate commits with intentionally poor commit messages. You will fix these in Checkpoint 6:

1. First commit: only the test file fix
   - Message: `"fix"`
2. Second commit: only the `.gitignore` changes
   - Message: `"update"`
...
That commit added excessive debug logging throughout the validator module. It clutters output and does not belong in production.
...
Edit the file to resolve the conflict, stage it, then run `git cherry-pick --continue`. For this checkpoint, keep both the existing code on your feature branch and the new `is_positive` function.
