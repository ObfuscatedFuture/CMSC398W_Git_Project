...
This role focuses on the modulo operation feature and fixing validation issues.

## Starting Point

Your starter branch: `starter/role-A`

This branch has 3 commits with various changes and some problems to fix.

## Workflow

Make sure you have cloned your group's fork of this repository (not the original upstream repo).

1. Checkout your starter branch (`starter/role-A`).
...
Look at the test output and the test file. Figure out why the tests are failing and fix the issue.

Task 2: Add a docstring (`src/calculator.py`)

Add a module-level docstring at the top of the file (before any imports) describing what the module does. Write it yourself.

### How to commit

Make these as two separate commits with intentionally poor commit messages. You will fix these in Checkpoint 6:

1. First commit: only the test fix
   - Message: `"fix"`
